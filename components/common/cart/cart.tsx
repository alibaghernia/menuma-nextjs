...
export const Cart: ICart = (props) => {
        ...
        return (
          <FlexItem key={key}>
            <FlexBox
              direction="column"
              gap={2}
              className="border border-black/[0.05] p-[1rem] rounded-[1.5rem]"
            >
              <FlexItem>
                ...
                >
                  <FlexItem>
                    <div className="text-[.7rem] font-bold whitespace-nowrap">
                      {orderItem.count > 1
                        ? `${orderItem.price.toLocaleString('IR-fa')} * ${
                            orderItem.count
                          }`
                        : orderItem.price.toLocaleString('IR-fa')}
                    </div>
                    ...
                    >
                      <FlexItem>
                        <div className="text-[1rem] font-bold bg-typography text-white px-[.7rem] py-[.2rem] rounded-full">
                          {(orderItem.price * orderItem.count).toLocaleString(
                            'IR-fa',
                          )}{' '}
                          ت
                        </div>
                  ...
                  <FlexItem>
                    <FlexBox gap={2} alignItems="center">
                      ...
                      <FlexItem className="w-7 h-7 relative">
                        <Container center>{orderItem.count}</Container>
                      </FlexItem>
                      <FlexItem
                        className={classNames(
                          'relative w-14 h-7 flex items-center justify-center bg-more rounded-lg cursor-pointer active:scale-[.8] transition duration-[.2s] select-none',
                        )}
                        onClick={() => {
  ...
  const getCartSum = () => {
    const prices = orderItems.map(
      (orderItem) => orderItem.price * orderItem.count,
    );
    ...
    <>
      ...
      >
        <FlexBox direction="column" className="h-full">
          ...
          <FlexItem grow={false}>
            <div className="text-[1.5rem] font-bold text-center w-full">
              سفارشات شما
            </div>
          </FlexItem>
          ...
          <FlexItem
            grow={false}
            className={classNames('mt-[1.5rem]', { hidden: !getCartSum() })}
          >
            <FlexBox gap={2} alignItems="center" className="whitespace-nowrap">
              <FlexItem>
                <div className="text-[1rem]">مبلغ قابل پرداخت</div>
              </FlexItem>
              <FlexItem grow>
                <hr className="w-full" />
              </FlexItem>
              <FlexItem>
                <div className="text-[1rem]">{getCartSum()} تومان</div>
              </FlexItem>
            </FlexBox>
          </FlexItem>
