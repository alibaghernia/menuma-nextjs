...
export const ConfirmModal: IConfirmModal = ({
    ...
    return createPortal(
        <>
            {open &&
                (<Container position='fixed' center className='z-[52] p-4 max-w-xs w-full bg-white rounded-[2rem]'>
                    <FlexBox direction='column' className='gap-6'>
                        {title && (
                            <FlexItem grow={false} className='text-center font-bold text-[1.2rem]'>
                                {title}
                            </FlexItem>
                        )}
                        <FlexItem grow>
                        ...
                        <FlexItem grow={false}>
                            <FlexBox gap={2}>
                                {dismissButton && (
                                    <FlexItem className='px-2 py-2 text-center rounded-[2rem] cursor-pointer' onClick={() => {
                                        onClose?.()
                                        onDismiss?.()
                                    }} grow>
                                        {dismissText}
            ...
            < Container position='fixed' className={
                classNames(
