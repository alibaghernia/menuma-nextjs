...
export default function ProductPage() {
    ...
    const incressOrderItemCount = useCallback((price: any) => {
        const key = `${product.id}-${price.id}`
        setOrderedItems(items => ({
            ...items,
            [key]: {
                count: orderedItems[key]?.count + 1
            }
        }))
    }, [orderedItems, product.id])

    const decressOrderItemCount = useCallback((price: any) => {
        const key = `${product.id}-${price.id}`
        if (orderedItems[key]?.count == 1) {
            console.log("delete");
            setOrderedItems(Object.fromEntries(Object.entries(orderedItems).filter(([okey, value]) => okey != key)))
        } else
            setOrderedItems(items => ({
                ...items,
                [key]: {
    ...
    const prices = useMemo(() => product.prices.map((price, key) => {
        ...
        const order = orderedItems[`${product.id}-${price.id}`];
        ...
        return (<div className="flex justify-between p-[.5rem] px-[1.5rem] rounded-[2rem] bg-more/[.1] items-center" key={key}>
            ...
            {!order ? (
                ...
                <div className="flex items-center gap-2">
                    <div className="w-7 h-7 flex items-center justify-center bg-more rounded-lg cursor-pointer active:scale-[.8] transition duration-[.2s]" onClick={() => incressOrderItemCount(price)}>+</div>
                    {order.count}
                    <div className="w-7 h-7 flex items-center justify-center bg-more rounded-lg cursor-pointer active:scale-[.8] transition duration-[.2s]" onClick={() => decressOrderItemCount(price)}>-</div>
                </div>
            )}
        </div>)
    }), [product, orderedItems, orderItem, decressOrderItemCount, incressOrderItemCount])
