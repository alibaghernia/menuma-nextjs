...
function Search() {
    ...
    const handleFetchBusinesses = (searchPhrase: string) => {
        ...
        axios.get(`/api/cafe-restaurants?all_fields=${searchPhrase}`)
            ...
            .then(({ data }) => {
                setFetchedItems((data as any[]).map((business) => ({
    ...
    const fetchNearBusinesses = () => {
        setLoading(true)
        axios.get(`/api/cafe-restaurants?lat=${params.lat}&long=${params.long}&distance=2000`)
            .finally(() => {
                setLoading(false)
            })
            .then(({ data }) => {
    ...
    const renderBusinesses = () => {
        ...
        return fetchedItems.map((business, idx) => (
            <div className="flex p-[1.13rem] gap-[.63rem] shadow-[0_0_10px_0_rgba(0,0,0,0.15)] rounded-[1rem] w-full md:w-full overflow-hidden" key={idx}>
                <Link href={`/${business.slug}`} className="relative w-[5.875rem] h-[5.875rem] shrink-0 rounded-full overflow-hidden border">
                    <Image fill src={business.logo} alt={business.title} />
                </Link>
                <div className="flex flex-col justify-between w-full">
                    <Link href={`/${business.slug}`} className="text-typography text-[1rem]">
                        {business.title}
                    </Link>
            ...
            <div className="mx-auto md:w-fit mt-[2.38rem] flex flex-col px-[2rem]">
                ...
                <div className="mt-[2.12rem]">
                    {!isNear ? (
                        ...
                        <div className="text-center text-typography/[.8] w-full">در شعاع 2 کیلومتری شما</div>
                    )}
                </div>
